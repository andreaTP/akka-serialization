...
object Picklables {
  ...
  def getAll(c: Context) = {
    ...
    val filtered = comps.filterNot( x =>
      ...
      x.getName.startsWith("scala.collection.concurrent") ||
      x.getName.startsWith("scala.collection.parallel") ||
      //probably less then this....
      x.getName.startsWith("scala.collection.mutable") ||
      x.getName.startsWith("akka.actor.TypedActor") ||
      //on Scala.js obviously...
      x.getName.startsWith("jawn") ||
      //?????
      x.getName.startsWith("akka.remote.StaticUPickleSerializer$$") ||
